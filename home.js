...
function getBaseUrl() {
  ...
  let base = meta?.getAttribute("content") || import.meta?.env?.BASE_URL || "/";
  if (!base.endsWith("/")) base += "/";
  return base;
}
var BASE = getBaseUrl();
var state = { items: [], filtered: [], category: "ALL", query: "" };
var grid = document.getElementById("grid");
var tabs = document.getElementById("tabs");
var q = document.getElementById("q");
function render() {
  if (!grid) return;
  grid.innerHTML = state.filtered.map(
    (item) => `
...
function renderTabs(categories) {
  if (!tabs) return;
  tabs.innerHTML = ["ALL", ...categories].map(
    (cat) => `
...
tabs?.addEventListener("click", (e) => {
  ...
  const cats = Array.from(tabs.querySelectorAll("button.tab")).map((b) => b.dataset.cat).filter((c) => c !== "ALL");
  renderTabs(cats);
  applyFilters();
});
